...
public:
    ...
    protected:
        ownership(mutex *own):_owning(own) {}
        mutex *_owning = nullptr;
...
///implements multiple mutex locking
/**
...
public:
    ...
    using ownership = std::array<mutex::ownership, n>;
...
template<int n>
multi_lock(mutex *(&list)[n]) -> multi_lock<n>;
